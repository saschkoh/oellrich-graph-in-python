...
class TestGraph(TestCase):
    ...
    def test_init_file(self):
        ...
        self.assertEqual(graph.edges[0].i_head, 0)
        self.assertEqual(graph.edges[0].i_tail, 1)
        self.assertEqual(graph.edges[1].name, "BC")
        self.assertEqual(graph.edges[1].i_head, 1)
        self.assertEqual(graph.edges[1].i_tail, 2)
        self.assertEqual(graph.edges[-1].name, "HI")
        self.assertEqual(graph.edges[-1].i_head, 7)
        self.assertEqual(graph.edges[-1].i_tail, 8)
        # check neighbours
